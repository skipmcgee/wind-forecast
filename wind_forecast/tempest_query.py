...
async def query_tempest(tempest_station_name: str, tempest_token: str, tempest_station: str = "134520", time_start_epoch: float = 0.0, time_end_epoch: float = 0.0, DEBUG: bool = False) -> tuple:
    ...
    if status_code == 200:
        ...
        json_object = {
            ...
            "dateTime": datetime.datetime.fromtimestamp(json_object['obs'][0][0]).strftime("%Y-%m-%d %H:%M:%S"),
            "wind": {
...
if __name__ == "__main__":
    ...
    response, status = loop.run_until_complete(query_tempest(tempest_station_name='Sandia Soaring Peak Tempest', tempest_token=tempest_token, tempest_station=tempest_station, DEBUG=True))	